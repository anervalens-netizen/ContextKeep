...
function fixtureRepository() {
  const directory = mkdtempSync(path.join(tmpdir(), "contextkeep-public-paths-"));
  const command = (args) => execFileSync("git", args, {
    cwd: directory, encoding: "utf8", env: {
      ...process.env, GIT_AUTHOR_NAME: "Synthetic Runner", GIT_COMMITTER_NAME: "Synthetic Runner",
      GIT_AUTHOR_EMAIL: "runner@users.noreply.github.com", GIT_COMMITTER_EMAIL: "runner@users.noreply.github.com",
    },
  });
  command(["init", "-q"]);
...
function capturedCheck(args, directory) {
  ...
  let text = ""; let code;
  console.log = (value) => { text += String(value); };
  try { code = checkRepository(args, directory); } finally { console.log = previous; }
  return { code, result: JSON.parse(text) };
}
{
  const { directory, command } = fixtureRepository();
  writeFileSync(path.join(directory, "harmless.bin"), Buffer.from([0, 1, 2, 3]));
  command(["add", "."]);
  command(["commit", "-qm", "Add synthetic binary"]);
  assert.equal(capturedCheck(["--history", "HEAD"], directory).code, 0);
  writeFileSync(path.join(directory, "leaked.sqlite"), Buffer.from([0, 1, 2, 3]));
  command(["add", "leaked.sqlite"]);
  for (const args of [[], ["--staged"]]) {
    const checked = capturedCheck(args, directory);
    assert.equal(checked.code, 1);
    assert.ok(checked.result.issues.some((issue) => issue.path === "leaked.sqlite" && issue.kind === "private-file-path"));
  }
...
{
  const { directory, command } = fixtureRepository();
  writeFileSync(path.join(directory, "leaked.sqlite"), "shared synthetic content\n");
  command(["add", "."]);
...
console.log("PASS: binary path rules and every historical blob alias remain enforced.");
